...
- Safe area (2026-09-15): `Displays::safeAreaInsets` di JUCE torna a zero abbastanza spesso da mandare la status row sotto la Dynamic Island. Aggiunta `vp::windowSafeAreaInsets()` (`IosMicPermission.mm`) che legge `safeAreaInsets` dalla key window della scena in primo piano; `MainComponent::effectiveSafeArea()` prende il massimo fra quella e il valore JUCE. **Da verificare su device** — è l'unico pezzo non provato a video.
