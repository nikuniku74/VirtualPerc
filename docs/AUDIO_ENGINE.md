...
**A cut of two quarters is a re-entry, not a restart.** The epoch watcher needs about four seconds of quiet before it will throw the decoder's evidence away, so a one-second mute never called it, and must not: the tempo is still right, only the count now sits on the three. `maybeDetectBarReentry` watches the analysis block peak against the recent loud level (a mute drops it in one callback; a fill does not) and opens a four-bar coming-in window on `BeatTracker`. Seek does the same via `notifyTrackSeek` without incrementing `analysisEpoch`. One `rotateBarIndex` per return. `VPTests --bar`. The clap reads `barTrusted` from that histogram (or the listener's lock) instead of a timer since the last rotation.
