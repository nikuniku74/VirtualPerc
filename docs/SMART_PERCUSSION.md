...
|---|---|---|---|---|
| **B1** | Percussioni | `1/16` produceva ottavi, non sedicesimi | ✅ fatto | R4 |
| **B2** | Clock | `pulseBeatInBar` sbagliato per le suddivisioni dopo il wrap del beat | ✅ fatto | R3 |
| **B3** | Percussioni | voice stealing brutale → click sui campioni | ✅ fatto | R4 |
| **B4** | Analisi | `subtractSpeakerLeak` trattava solo i primi 2048 campioni → gradino nel segnale d'analisi | ✅ fatto | R1/R2 |
...
Legenda: ✅ corretto · 🔴 blocca un requisito · 🟠 lo degrada · 🟡 debito ·
🔵 vincolo di piattaforma.

---

## 3. Difetti già presenti nel codice — **corretti**

Questi andavano chiusi **prima** di aggiungere intelligenza: sono le regressioni
che altrimenti si portano dietro ogni funzione nuova. Sono stati corretti tutti e
cinque; sotto resta la descrizione del difetto, seguita da cosa è stato fatto e
dal test che lo blocca. I riferimenti a riga sono quelli del codice **prima**
della correzione.
...
**Fatto**: la tabella del pattern è ora a 16 step per battuta e ogni impulso ci
viene mappato con `barBeat * 4 + (idx * 4) / groovePulses`. Su `1/4` e `1/8`
cade esattamente sugli stessi step di prima — quelle due griglie sono immutate —
e `1/16` smette di scartare gli impulsi dispari. Le congas restano sugli ottavi
perché i sedicesimi dispari della tabella non ne portano nessuna.

**Test**: `grid-density` — 4 battute a 120 BPM danno 16 / 32 / 64 colpi di shaker
su `1/4` / `1/8` / `1/16`. Sul codice precedente `1/16` ne dava 31.
...
**Fatto**: indice di impulso e battuta si ricavano entrambi da `p`, che conta gli
impulsi dalla fase zero del beat in corso a inizio blocco: `idx = p % pulsesPerBeat`
e `barBeat = beatAtStart + p / pulsesPerBeat`. Non c'è più nessun caso speciale
legato a `wrappedBeat`.

**Test**: `bar-pulse` — otto combinazioni di tempo, suddivisione e blocco
(60–210 BPM, 256–4096 campioni), 20 s ciascuna: `barPulse` deve avanzare di
esattamente uno modulo la battuta. Sul codice precedente falliva.
...
**Fatto**: `stealKind` è diventato `releaseKind`, che avvia una rampa di release
di 2.5 ms **a partire dall'offset di campione della nota nuova**, invece di
spegnere la voce fra due campioni. L'allocazione passa da `allocateVoice()`, che
prende uno slot libero e, solo se non ce n'è, la voce più silenziosa fra quelle
che hanno già suonato — il comportamento che `docs/AUDIO_ENGINE.md` dichiarava da
sempre — contando l'evento in `hardSteals()`.

**Test**: `voice-steal` — al riattacco devono esserci due voci vive (la vecchia in
rampa e la nuova), una sola dopo 8 ms, e su 30 s a 200 BPM in `1/16` nessun
`hardSteal`. Sul codice precedente le voci al riattacco erano una.

Una nota onesta sulla metrica: il primo tentativo misurava il massimo gradino
dell'uscita, e **non discriminava** — il gradino introdotto dal taglio (~0.4) sta
sotto il transiente naturale del grano (0.81). Il conteggio delle voci vive è
l'osservabile che distingue davvero i due comportamenti.
...
**Fatto**: lo scratch è allocato in `prepare()` alla dimensione di `maxBlock` e la
sottrazione copre l'intero blocco.

**Test**: `leak-residual` — il microfono riceve l'uscita dell'app ritardata, e si
misura quanto ne resta dopo la sottrazione (`analysisPeak / inputPeak`) a blocco
1024 e 4096. Corretto: 0.62 e 0.57. Con il troncamento: 0.62 e 0.79.

Il test ha richiesto un ritardo di 150 ms, più lungo del blocco. Non è un
artificio: con 85 ms di blocco e 20 ms di ritardo acustico, la coda del blocco
andrebbe cancellata con uscita non ancora prodotta, e il canceller legge oltre il
puntatore di scrittura del ring. È un limite di causalità reale del design
attuale, che sul device non si presenta (buffer da 256 campioni, ~20 ms di
ritardo) ma che va tenuto presente prima di alzare la dimensione del buffer.
...
**Fatto**: il worker confronta `fifo.droppedSamples()` a ogni giro; se è cambiato,
resetta resampler, feature ed LSTM e chiama `BeatDecoder::notifyDiscontinuity`,
che avanza la propria linea temporale dei secondi persi, svuota la storia dei beat
e azzera le catene di evidenza — **tenendo** tempo, regime e livello metrico. Il
riempimento extra che le feature richiedono dopo un reset (`frameLen - hop`) entra
in `analysisSampleFor`, altrimenti ogni frame successivo verrebbe marcato in
anticipo di 44 ms.

**Test**: `fifo-overrun` verifica che la FIFO riporti esattamente i campioni
sovrascritti; `gap-decoder` che dopo un buco di 2.3 s (non un numero intero di
beat) il BPM non si muova e il lock resti valido, ma la **confidenza scenda**
(1.00 → 0.59) e si ricostruisca. È quest'ultima la parte che discrimina: senza il
fix il decoder continua a dichiarare 1.00 su evidenza che non ha più.
